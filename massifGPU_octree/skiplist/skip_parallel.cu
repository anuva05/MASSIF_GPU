...
__device__ static Node *node_search(Skiplist *sl, OctreeNode* elem, int desired_level);
__device__ static Node *node_create(OctreeNode* val, int level);
...
__global__ static void skiplist_gather_internal(OctreeNode *dest, Skiplist *sl);

/* Initializes the skiplist to contain nothing but an empty head node */
__global__ static void create_head(Skiplist *sl)
{ 
  OctreeNode* temp;
  sl->head = node_create(temp, MAX_LEVEL);
...
OctreeNode* skiplist_gather(Skiplist *sl, int *dim)
{
  int size = skiplist_size(sl);
  OctreeNode *dest_dev;
  OctreeNode *dest = (OctreeNode *)malloc(size * sizeof(OctreeNode));

  CHECK(cudaMalloc(&dest_dev, size * sizeof(OctreeNode)));

  skiplist_gather_internal<<<1, 1>>>(dest_dev, sl);
  CHECK(cudaDeviceSynchronize());
  CHECK(cudaMemcpy(dest, dest_dev, size * sizeof(OctreeNode), cudaMemcpyDeviceToHost));
