...
//Creates an octree and copies it to the GPU
void createOctreeOnGPU(){

cudaError_t cudaStatus;
OctreeNode* nodes[16];
OctreeNode* octreeGPU;

cudaMalloc((void**)&octreeGPU, sizeof(OctreeNode)*16);
...
nodes[i]->startX = ((NX-1)/2);
nodes[i]->startY = 0;
nodes[i]->startZ = (NZ-1)/2;
nodes[i]->size= NX/2;
nodes[i]->ds = 4;
i=i+1;
}

cudaMemcpy(octreeGPU,nodes,sizeof(OctreeNode)*16, cudaMemcpyHostToDevice);
