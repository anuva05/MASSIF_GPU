...
__device__ void sample_stage1(void *dataOut, size_t offset, cufftDoubleComplex element, void *callerInfo, void *sharedPtr) {
  ...
  //num samples in each plane (Excluding K x K domain) = (NX*NY - K*K)/DS^2
  ((cufftDoubleComplex*)dataOut)[offset] = element;
   printf("element: %lf\n", element.x );
  //printf("OFFSET:%d, X,Y: %d,%d, Z plane :%d\n", off, dim0, dim1 ,dim2 );
  ...
  //if point is outside domain but valid sample
  if(dim0%DS == 0 && dim1%DS == 0) {

    if((dim2< K)&&(dim0>=K || dim1 >=K)){
