...
//IN THIS CODE: Samples are not removed from the x and y dimensions, but only zero-ed out
//Samples are removed from z dimension
//This approach saves memory
//Single precision = cufft and fftw will differ slightly for small value
//Answer matching with FFT for the planes that are to be kept.
#include <iostream>
#include <string.h>
#include <math.h>
using namespace std;
...
#define B 128 // Number of pencils in one batch
#define NRANK 3
...
//if 'z' value is not in 0 to K or not one of the pre-specified planes, then set it to 0.
  ...
  if ((z%DS== 0)&&(z>=K)){
    ((cufftComplex*)dataOut)[B*(K + (z/DS)-1) + el]= element;
  }
}
...
__device__ void sample_stage1(void *dataOut, size_t offset, cufftComplex element, void *callerInfo, void *sharedPtr) {
  ...
  if (((dim0>=0 && dim0 <K)&&(dim1>=0 && dim1 <K))||(dim0%DS == 0 && dim1%DS == 0)){
    ((cufftComplex*)dataOut)[(1 * dim0) + (NX* dim1) + (NX*NY*dim2)] = element;
  }
  else{
    r.x= 0.0;
    r.y= 0.0;
    ((cufftComplex*)dataOut)[(1 * dim0) + (NX*dim1) + (NX*NY*dim2)] = r;
  }
}
...
void print_XYplanes( cufftComplex* arr,int X, int Y, int Z){
...
cudaMalloc((void**)&d_temp3, sizeof(cufftComplex)*B*(K + (NZ-K)/DS));
if (cudaGetLastError() != cudaSuccess){
fprintf(stderr, "Cuda error: Failed to allocate\n");
exit(-1);
}
...
//  //Copy out output
//
  num_samples =  NX*NY*(K + (NZ-K)/DS); //(K + (NX-K)/DS)*(K + (NY-K)/DS)*(K + (NZ-K)/DS);
  cudaStatus = cudaMemcpy(result, d_result, sizeof(cufftComplex)*(num_samples), cudaMemcpyDeviceToHost);
 	//cudaStatus = cudaMemcpy(result, d_inv_stage1, sizeof(cufftComplex)*(num_samples), cudaMemcpyDeviceToHost);
 	if (cudaStatus != cudaSuccess){
...
int main(int argc, char **argv){
  ...
  int final_samples;
  final_samples =   NX*NY*(K + (NZ-K)/DS); // ( K + (NX-K)/DS )*(K + (NY-K)/DS)*(K + (NZ-K)/DS);;
...
for(int i=0;i<NZ;i++){
  for(int j=0;j<NY;j++){
   ...
   //Printing the k^th plane to compare
   k=1;
   count = NX*NY*(k-1);
   while(count<NX*NY*k){
      cout<< count << ":" << result[count].x << ", " <<result[count].y << "\t FFTW:" << fftwf_output[2*count] <<"," << fftwf_output[2*(count+1)] << endl;
      count = count + 1;
    }
